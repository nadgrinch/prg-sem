...
int xwin_init(int w, int h)
{
   ...
   assert(win == NULL);
   win = SDL_CreateWindow("PRG Semester Project", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, w, h, SDL_WINDOW_SHOWN);
   assert(win != NULL);
   SDL_SetWindowTitle(win, "DRATVSIM SEM");
...
int xwin_msg(const char *msg, const char *title){
   int r = SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, title, msg, win);
   return r;
}
...
int xwin_parameters(void){
   ...
   SDL_MessageBoxData data = {
      .window = win,
      .message = "Choose parameter you want to change or QUIT parameter change",
      .buttons = buttons, 
      .colorScheme = NULL, 
...
int xwin_change_param(int buttonid, double *parameter){
   int ret;
   char *name[] = {"re_max","re_min","im_max","im_min","c_re","c_im"};
   char text[35];
   sprintf(text,"Choose change of %s of value %.2f",name[buttonid],*parameter);
   SDL_MessageBoxButtonData buttons[] = {
      {1,0,"+0.01"},
      {1,1,"+0.1"},
      {1,2,"-0.01"},
...
int xwin_change_n(int *n){
   ...
   SDL_MessageBoxData data = {
      .window = win,
      .message = "Increase or decrease parameter n",
      .buttons = buttons,
      .numbuttons = SDL_arraysize(buttons),
      .title = "Parameter Change",
      .flags = 0,
   ...
   int ret = SDL_ShowMessageBox(&data,&buttonid);
...
int xwin_animate(void){
   ...
   SDL_MessageBoxData data  = {
      .window = win,
      .message = "Animation of increasing or decreasing value of parameter c\nChoose animation direction",
      .buttons = buttons,
      .flags = 0,
      .colorScheme = NULL,
...
void xwin_save(void){
   ...
   SDL_MessageBoxData data = {
      .window = NULL,
      .message = "Choose format used to saving the image\n",
      .buttons = buttons,
      .numbuttons = SDL_arraysize(buttons),
      .title = "Save Image",
      .colorScheme = NULL,
   ...
   SDL_ShowMessageBox(&data,&format);
   SDL_Surface *sur = SDL_GetWindowSurface(win);
   switch (format) {
      case 0:
         IMG_SaveJPG(sur,"image.jpg",66);
         break;
      case 1:
         IMG_SavePNG(sur,"image.png");
         break;
   }
}
